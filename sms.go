...
func (c *QcloudSMS) SendSMSSingle(ss SMSSingleReq) error {
	...
	c = c.NewSig(ss.Tel.Mobile, timestamp).NewURL(SENDSMS)
...
func (c *QcloudSMS) SendSMSMulti(sms SMSMultiReq) (bool, error) {
	...
	timestamp := time.Now().Unix()
	c = c.NewSig(mobileStr, timestamp).NewURL(MULTISMS)
...
func (c *QcloudSMS) GetStatusForMobile(smr StatusMobileReq) (StatusMobileResult, error) {
	timestamp := time.Now().Unix()
	c = c.NewSig("", timestamp).NewURL(MOBILESTATUS)
...
func (c *QcloudSMS) GetStatusMQ(psr PullStatusReq) (StatusMobileResult, error) {
	timestamp := time.Now().Unix()
	c = c.NewSig("", timestamp).NewURL(PULLSTATUS)
